    ...
    fetch("http://localhost:8090/fama-market/api/subcategories/all", {headers})
        ...
        .then((data) => {
            ...
            familiasMap.forEach((categoriasMap, familiaName) => {
                ...
                const categoriaId = obtenerIdDeLaURL();

                categoriasMap.forEach((subcategoriasArray, categoriaName) => {
                    const categoriaItem = document.createElement("li");
                    categoriaItem.innerHTML = `<a href="Views/productos.html?categoriaId=${categoriaId} target="_blank"">${categoriaName}</a>`;
...
function obtenerIdDeLaURL() {
    // Implementa la lógica para obtener el ID de la categoría desde la URL
    // Puedes usar window.location o cualquier otro método según tu aplicación
    // En este ejemplo, devolveré un ID ficticio (1) para propósitos de demostración.
    return
}