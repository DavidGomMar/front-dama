...
if (existingToken) {
    // Si hay un token, obtener el perfil almacenado
    const existingProfile = sessionStorage.getItem("profile");
    
    // Redirigir según el perfil
    if (existingProfile == 1) {
        // Perfil de usuario, redirigir al index normal
        window.location = "./../index.html";
    } else if (existingProfile == 2) {
...
btn.addEventListener("click", async function (event) {
    ...
    try {
        ...
        // Redirigir según el perfil
        if (jsonResponse.perfil == 1) {
            // Perfil de usuario, redirigir al index normal
            window.location = "./../index.html";
        } else if (jsonResponse.perfil == 2) {
            // Perfil de administrador, redirigir al index_admin
            window.location = "./../index_admin.html";
        } else {
            // Otro perfil, manejar según sea necesario
