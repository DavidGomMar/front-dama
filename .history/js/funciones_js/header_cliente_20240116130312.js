...
familiasMap.forEach((categoriasMap, familiaName) => {
    ...
    categoriasMap.forEach((subcategoriasArray, categoriaObject) => {
        const categoriaItem = document.createElement("li");
        const categoriaName = categoriaObject.nameCategory;
        const categoriaId = categoriaObject.categoryId;
        categoriaItem.innerHTML = `<a href="Views/productos.html" data-categoria-id="${categoriaId}">${categoriaName}</a>`;

        if (Array.isArray(subcategoriasArray) && subcategoriasArray.length > 0) {
            ...
            subcategoriasArray.forEach((subcategoriaObject) => {
                const subcategoriaItem = document.createElement("li");
                const subcategoriaName = subcategoriaObject.nameSubCategory;
                const subcategoriaId = subcategoriaObject.subCategoryId;
                // Añadir el ID de la subcategoría como parámetro en la URL
                subcategoriaItem.innerHTML = `<a href="Views/productos.html" data-categoria-id="${categoriaId}" data-subcategoria-id="${subcategoriaId}">${subcategoriaName}</a>`;
