...
function confirmarEliminarProductoConImagenes(productId) {
    ...
    // Función para eliminar todas las imágenes del producto
    function eliminarTodasLasImagenesDelProducto() {
        return fetch(`http://localhost:8090/fama-market/api/images/delete/all/${productId}`, {
            method: 'DELETE',
            headers: headers,
        })
            .then((response) => {
    ...
    // Lógica combinada
    eliminarTodasLasImagenesDelProducto()
        .then(() => eliminarProducto())
        .then(() => obtenerListaProductos())
        .catch((error) => {
...
$("#btnConfirmarEliminar").on("click", function () {
    ...
    // Llamar a la función combinada para eliminar imágenes y producto
    confirmarEliminarProductoConImagenes(productId);
});
