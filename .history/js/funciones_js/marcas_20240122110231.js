...
function mostrarModalEliminar(idMarca) {
    ...
    if (idMarca) {
        ...
        console.error('ID de la Marca a Eliminar no válido.');
    }
}
...
if (idMarca !== null) {
    ...
    // No llames a mostrarModalEliminar aquí
} else {
    console.error('No se puede mostrar el modal. ID de la Marca a Eliminar no encontrado.');
}

// Agrega un evento al clic del botón para llamar a la función mostrarModalEliminar
$('#botonAbrirModalEliminar').on('click', function() {
...
function eliminarMarca() {
    ...
    // Verificar si el ID de la marca está disponible
    if (brandIdAEliminar) {
        const token = sessionStorage.getItem("jwt");
