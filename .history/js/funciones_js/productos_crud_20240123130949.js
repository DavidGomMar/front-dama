...
$("#btnConfirmarEliminar").on("click", function () {
    ...
    eliminarImagenesProducto(productId)
        .then(() => {
...
function eliminarImagenesProducto(productId) {
    // Obtener el token desde sessionStoreNg
    ...
    // Realizar la llamada a la API para eliminar las imágenes del producto
    return fetch(`http://localhost:8090/fama-market/api/images/delete/${productId}`, {
        ...
        .then((response) => {
            if (!response.ok) {
                throw new Error(`Error al eliminar las imágenes: ${response.statusText}`);
            }
            ...
            if (!response.ok) {
                if (response.status === 403) {
                    ...
                    return response.json().then((errorData) => {
                        // Devolver un objeto de error más informativo
