...
btn.addEventListener("click", async function (event) {
    ...
    try {
        ...
        // Realiza la redirección según el perfil
        if (userRole === 1) {
            window.location = "./../index.html";  // Redirección para usuarios
        } else if (userRole === 2) {
            window.location = "./../index_admin.html";  // Redirección para administradores
        } else {
            // En caso de un perfil no reconocido, puedes manejarlo de acuerdo a tus necesidades
            console.error("Perfil no reconocido:", userRole);
        }
    } catch (error) {
