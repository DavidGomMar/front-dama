    ...
    fetch("http://localhost:8090/fama-market/api/subcategories/all", { headers })
        ...
        .then((data) => {
            console.log("Datos recibidos de la API:", data);
             ...
             // Obtén una lista de categoryId únicos
        const uniqueCategoryIds = [...new Set(data.map(item => item.category.categoryId))];

        // Realiza alguna lógica para seleccionar un categoryId (por ejemplo, el primer elemento)
        const categoryId = uniqueCategoryIds.length > 0 ? uniqueCategoryIds[0] : null;
            ...
            const categoryId = data.length > 0 ? data[0].category.categoryId : null;
