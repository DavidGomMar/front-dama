...
function confirmarEliminarProducto(productId) {
    // Configurar el ID del producto para la eliminación
    $("#btnConfirmarEliminar").data("product-id", productId);

    // Mostrar el modal de confirmación
    $("#modalEliminar").modal("show");
}

function eliminarTodasLasImagenesDelProducto(productId) {
    ...
    if (!token) {
        ...
        return Promise.reject(new Error('No se encontró un token'));
    }
    ...
    // Realizar la llamada a la API para eliminar todas las imágenes del producto
    return fetch(`http://localhost:8090/fama-market/api/images/delete/product/${productId}`, {
        method: 'DELETE',
        headers: headers,
    })
        .then((response) => {
            if (!response.ok) {
                throw new Error(`Error al eliminar todas las imágenes: ${response.statusText}`);
            }
            console.log('Todas las imágenes del producto eliminadas exitosamente');
        });
}
function eliminarProducto(productId) {
    ...
    return fetch(`http://localhost:8090/fama-market/api/products/delete/${productId}`, {
        method: 'DELETE',
        headers: headers,
    })
        .then((response) => {
            if (!response.ok) {
                if (response.status === 403) {
                    ...
                    return response.json().then((errorData) => {
                        throw new Error(`Error al eliminar el producto: ${response.statusText} - ${errorData.message}`);
                    });
                }
            }
...
$("#btnConfirmarEliminar").on("click", function () {
    // Obtener el ID del producto desde el botón de confirmación
    const productId = $(this).data("product-id");

    // Eliminar todas las imágenes del producto
    eliminarTodasLasImagenesDelProducto(productId)
        .then(() => {
            // Después de eliminar imágenes, eliminar el producto
