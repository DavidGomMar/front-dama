document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("http://localhost:8090/fama-market/api/subcategories/all", { headers })
        ...
        .then((data) => {
            ...
            // Obtén el categoryId del primer elemento del array data
            const categoryId = obtenerCategoryIdDeAPI(data);
            ...
            familiasMap.forEach((categoriasMap, familiaName) => {
                ...
                categoriasMap.forEach((subcategoriasArray, categoriaName) => {
                    ...
                    const categoryId = obtenerCategoryIdDeAPI(subcategoriasArray);;
                    categoriaItem.innerHTML = `<a href="Views/productos.html?categoriaId=${categoryId}" target="_self">${categoriaName}</a>`;
...
function obtenerCategoryIdDeAPI(subcategoriasArray) {
    // Verifica si subcategoriasArray tiene elementos
    if (subcategoriasArray.length > 0) {
        // Accede a la propiedad "category" del primer elemento
        const firstSubcategoria = subcategoriasArray[0];
        // Verifica si el elemento tiene la propiedad "categoryId"
        if (firstSubcategoria.category && firstSubcategoria.category.categoryId) {
            // Retorna el categoryId
            return firstSubcategoria.category.categoryId;
        }
    }
    // Retorna null si no se encuentra el categoryId
    return null;
}