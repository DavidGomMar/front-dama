...
function mostrarMarcasEnTabla(marcas) {
    ...
    marcas.forEach((marca) => {
        ...
        celdaAcciones.innerHTML =
            '<button type="button" class="btn btn-eliminar btn-sm" data-bs-toggle="modal" data-bs-target="#modalEliminarMarca" onclick="eliminarMarca(' + marca.brandId + ')">Eliminar</button>';
    });
}
...
.then(imagenBase64 => {
    ...
    // Cambiar la propiedad 'image' a 'imageName' y enviar el string base64 directamente
    const marcaData = {
        nameBrand: nombreMarca,
        imageName: imagenBase64,
    };

    // Realizar la solicitud POST para agregar una nueva marca con el nombre de la imagen base64
...
function eliminarMarca(id) {
    // Definir la variable global para almacenar el ID de la marca que se va a eliminar
    window.marcaAEliminarId = id;
...
function confirmarEliminarMarca() {
    // Obtener el ID de la marca a eliminar desde la variable global
    const id = window.marcaAEliminarId;
    ...
    // Llamar a la función para eliminar la marca con el ID proporcionado
    eliminarMarcaPorId(id);
}

function eliminarMarcaPorId(id) {
    // Obtener el token desde sessionStoreNg
    const token = sessionStorage.getItem('jwt');
    ...
    // Realizar la solicitud DELETE para eliminar la marca con el ID proporcionado
    fetch(`http://localhost:8090/fama-market/api/brands/delete/${id}`, {
        method: 'DELETE',
        headers: headers,
    })
    .then(response => {
    ...
    .catch(error => {
        console.error(error.message || 'Error desconocido al eliminar marca');
    });
}
