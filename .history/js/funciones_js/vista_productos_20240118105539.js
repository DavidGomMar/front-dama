document.addEventListener("DOMContentLoaded", function () {
    ...
    // Llama a la función correspondiente dependiendo de la presencia de "categoriaId"
    if (categoriaId) {
        mostrarProductosByCategoria(categoriaId); // Pasa categoriaId como parámetro
    } else {
        mostrarProductos();
    }
...
function obtenerListaProductosByCategoria(categoriaId) { // Agrega categoriaId como parámetro
    ...
    fetch(`http://localhost:8090/fama-market/api/products/findByIdSubmarca/${categoriaId}`, { headers })
        .then((response) => response.json())
        .then((productos) => {
            // Llama a la función para mostrar los productos en la tabla
            mostrarProductos(productos);
...
function mostrarProductosByCategoria(productos) {
    ...
    if (Array.isArray(productos)) {
        ...
        console.error("La respuesta del servidor no es un array:", productos);
    }
}
